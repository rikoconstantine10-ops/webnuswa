...
type SearchResult = {
  orders: { id: string; code: string; buyerName: string; status: string }[];
...
export default function DashboardTopbar({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch(`/api/dashboard/search?q=${encodeURIComponent(query.trim())}`)
        .then((r) => r.json())
  ...
  const hasResults = results && results.orders.length + results.products.length > 0;
      ...
      <div className="h-16 px-4 md:px-6 flex items-center gap-3">
        ...
        <Link href="/dashboard" className="shrink-0 flex items-center gap-2 mr-2">
          ...
          <span className="hidden sm:block font-extrabold text-slate-900 leading-tight">
            ...
            <span className="block text-[10px] font-semibold uppercase tracking-wide text-slate-400">Dashboard Seller</span>
          </span>
        </Link>
        ...
        <div ref={boxRef} className="relative flex-1 max-w-md">
          <input
            ...
            className="w-full bg-slate-100 border border-transparent focus:border-teal-300 focus:bg-white rounded-xl pl-9 pr-3 py-2 text-sm outline-none transition"
          />
              ...
              {!results ? (
                ...
                <div className="py-2">
                  {results.orders.length > 0 && (
                    <div className="px-2">
                      <p className="text-[10px] font-bold uppercase text-slate-400 px-2 py-1">Pesanan</p>
                      {results.orders.map((o) => (
                        <Link
                          key={o.id}
                          href={`/dashboard/orders?q=${encodeURIComponent(o.code)}`}
                          onClick={() => setSearchOpen(false)}
                          className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-teal-50 text-sm"
                        >
                          <span>🧾</span>
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-lg ring-1 ring-slate-900/10 overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                ...
                <Link href="/dashboard/notifications" onClick={() => setNotifOpen(false)} className="text-xs text-teal-600 hover:underline">
                  Lihat semua
                </Link>
              </div>
                ...
                {recentNotifications.length === 0 ? (
                  ...
                  recentNotifications.map((n) => (
                    <Link
                      ...
                      className={`flex items-start gap-2 px-4 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50 ${!n.readAt ? "bg-teal-50/40" : ""}`}
                    >
        ...
        <div ref={profileRef} className="relative shrink-0">
          ...
          >
            <span className="w-8 h-8 rounded-full bg-gradient-to-br from-teal-500 to-emerald-500 text-white flex items-center justify-center text-xs font-bold">
              {storeName.charAt(0).toUpperCase()}
            </span>
            <span className="hidden md:block text-xs text-slate-500 max-w-32 truncate">{storeName}</span>
          </button>
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl shadow-lg ring-1 ring-slate-900/10 overflow-hidden z-50 py-1">
              <div className="px-4 py-2.5 border-b border-slate-50">
                <p className="text-sm font-bold truncate">{storeName}</p>
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${STATUS_TONE[storeStatus] ?? "bg-slate-100 text-slate-500"}`}>
                    {STATUS_LABEL[storeStatus] ?? storeStatus}
                  </span>
                  {storePaused && (
              ...
              <a href={`/s/${storeSlug}`} target="_blank" rel="noreferrer" className="block px-4 py-2 text-sm hover:bg-slate-50">
                🔗 Lihat Toko
              </a>
              <Link href="/dashboard/store" onClick={() => setProfileOpen(false)} className="block px-4 py-2 text-sm hover:bg-slate-50">
                ⚙️ Pengaturan Toko
              </Link>
